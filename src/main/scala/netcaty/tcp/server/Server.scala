 ...
 */
class Server(ssl: Boolean, port: Int, responseLength: Int, handler: tcp.RequestHandler) {
  ...
  def start(stopAfterOneResponse: Boolean) = {
    ...
    serverChannel = (new ServerBootstrap)
      ...
      .childHandler(new PipelineInitializer(ssl, this, responseLength, handler, stopAfterOneResponse))
      .bind(port)
